...
export default function Home() {
  ...
  const cartIds = new Set(cart.map((book) => book.id))
  ...
  return <main className="store-shell">
    ...
    <section className="store-catalog" id="catalog">
      ...
      <div className="store-grid">{shown.map((book) => <article className="store-card" key={book.id}>
        ...
        <div className="store-buy"><strong>${book.price.toFixed(2)}</strong><button disabled={cartIds.has(book.id)} onClick={() => add(book)}>{cartIds.has(book.id) ? 'IN CART ✓' : 'ADD TO CART ＋'}</button></div>
      </article>)}</div>
    </section>
