...
const PRODUCTS=Object.fromEntries(books.map((book)=>[book.id,{name:book.title,amount:Math.round(book.price*100),description:book.tileDescription}]))
...
export async function POST(request){
 try{
  ...
  const params=new URLSearchParams({mode:'payment',success_url:`${origin}/success?session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${origin}/#catalog`,allow_promotion_codes:'true','metadata[store]':'cloudcertificationstore','metadata[format]':'pdf-epub'})
  ;[...counts].forEach(([code,quantity],index)=>{const product=PRODUCTS[code];params.set(`line_items[${index}][price_data][currency]`,'usd');params.set(`line_items[${index}][price_data][unit_amount]`,String(product.amount));params.set(`line_items[${index}][price_data][product_data][name]`,product.name);params.set(`line_items[${index}][price_data][product_data][description]`,product.description.slice(0,500));params.set(`line_items[${index}][quantity]`,String(quantity))})
